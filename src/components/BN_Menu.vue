<template>
    <div class="sticky top-0 w-full transition-all duration-1000 z-[200] overflow-hidden"
    :class="[scrolled ? 'drop-shadow-2xl' : 'drop-shadow-none', globalStore.loaded2 ? 'h-16 mt-0 sm:mt-24' : 'h-screen pointer-events-none']">

        <div class="fixed top-0 left-0 w-full h-full from-BN_BACKGROUND via-sky-950 to-indigo-950 flex justify-center items-center z-[200] text-5xl text-BN_FONT_WHITE font-bold transition-all duration-1000"
            :class="globalStore.loaded3 ? 'opacity-0 pointer-events-none' : 'opacity-100'">
            <svg :class="[globalStore.loaded2 ? 'left-[35px] sm:left-[156px] -mt-3 sm:-mt-[18px] h-6 sm:h-9 drop-shadow-lg' : 'h-24 -mt-12 -translate-y-1/2 left-1/2 drop-shadow-2xl', globalStore.loaded1 ? '' : 'animate-pulse']" class="absolute top-1/2 -translate-x-1/2 transition-all duration-1000" viewBox="0 0 133 82" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="absolute w-full h-full flex bg-gradient-to-r from-BN_BACKGROUND via-sky-950 to-indigo-950">
            <div class="grow h-full flex relative justify-center items-center" :class="globalStore.loaded3 ? 'opacity-100' : 'opacity-0'">
                ...
                <img loading="eager" class="absolute top-8 sm:-top-44 -mt-24 z-10 w-72 sm:w-auto transition-all duration-1000" :src="TopTitleBGElement"
                    :class="[scrolled ? '-right-9 sm:right-0' : '', globalStore.loaded3 ? 'opacity-100 -right-36 sm:-right-32' : 'opacity-0 -right-[680px]']">

                <button @click="scroll('contact')" class="right-4 sm:right-32 | absolute h-10 sm:h-12 w-36 sm:w-44 | rounded-xl text-sm sm:text-base | font-bold text-BN_FONT_WHITE hover:text-BN_BG_WHITE | flex justify-center items-center 
                    z-50 overflow-hidden border-4 group | hover:right-4 sm:hover:right-[128px] hover:border-0 
                    border-BN_PRIMARY hover:border-opacity-0 transition-all duration-500" :class="globalStore.loaded3 ? 'opacity-100' : 'opacity-0'">
                    <div class="absolute flex justify-center items-center h-0 w-0 opacity-0 bg-BN_PRIMARY -z-[1] rounded-2xl
                        group-hover:h-12 group-hover:w-44 group-hover:opacity-100 transition-all duration-500"></div>
                        Contact Us <i class="ml-1.5 group-hover:ml-3 fa fa-solid fa-circle-right transition-all duration-500"></i>
                </button>
                
            </div>
        </div>
        
        <div class="ml-4 sm:ml-32 | absolute flex h-full flex-col transition-all duration-500 overflow-hidden z-50" :class="globalStore.loaded3 ? 'w-64' : 'w-0'">
    ...
    methods: {
        handleScroll() {
            ...
            if (!window.mobileCheck())
                ...
                this.scrolled = scrollPosition > 1;
        },
        scroll(id) {

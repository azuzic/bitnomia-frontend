<template>
    ...
    :class="[scrolled ? 'drop-shadow-2xl' : 'drop-shadow-none', globalStore.loaded2 ? 'h-16 mt-24' : 'h-screen pointer-events-none']">
        ...
        <div class="fixed top-0 left-0 w-full h-full from-BN_BACKGROUND via-sky-950 to-indigo-950 flex justify-center items-center z-[200] text-5xl text-BN_FONT_WHITE font-bold transition-all duration-1000"
            ...
            <svg :class="[globalStore.loaded2 ? 'left-[156px] -mt-[18px] h-9 drop-shadow-lg' : 'h-24 -mt-[0px] -translate-y-1/2 left-1/2 drop-shadow-2xl', globalStore.loaded1 ? '' : 'animate-pulse']" class="absolute top-1/2 -translate-x-1/2 transition-all duration-1000" viewBox="0 0 133 82" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="absolute w-full h-full flex bg-gradient-to-r from-BN_BACKGROUND via-sky-950 to-indigo-950">
            <div class="grow h-full flex relative justify-center items-center" :class="globalStore.loaded3 ? 'opacity-100' : 'opacity-0'">
                ...
                <img loading="eager" class="absolute -right-32 -top-44 -mt-24 z-10 transition-all duration-1000" :src="TopTitleBGElement"
                    :class="[scrolled ? '-right-8' : '-right-32', globalStore.loaded3 ? 'opacity-100 -right-32' : 'opacity-0 -right-[680px]']">

                <button @click="scroll('contact')" class="absolute right-32 h-12 w-44 | rounded-xl text | font-bold text-BN_FONT_WHITE hover:text-BN_BG_WHITE | flex justify-center items-center 
                    z-50 overflow-hidden border-4 group | hover:right-[128px] hover:border-0 
        ...
        <div class="absolute flex h-full flex-col ml-32 transition-all duration-500 overflow-hidden" :class="globalStore.loaded3 ? 'w-64' : 'w-0'">
